...
#ifndef EFFICIENT_BHEAP_PRIO_QUEUE_HPP
#define EFFICIENT_BHEAP_PRIO_QUEUE_HPP
...
#ifdef __GNUC__
#define prio_q_likely(x)       __builtin_expect(!!(x), 1)
#define prio_q_unlikely(x)     __builtin_expect(!!(x), 0)
#else
  #define prio_q_likely(x) x
  #define prio_q_unlikely(x) x
#endif
...
~skip_vector() noexcept(std::is_nothrow_destructible<T>::value)
{
  ...
  {
    destroy();
    A::deallocate(*this, ptr_, storage_size_);
  }
}
...
{
  auto i = end_;
  while (prio_q_unlikely(i-- != 0))
  {
    if (prio_q_likely(i & block_mask))
    {
      A::destroy(*this, ptr_ + i);
    }
  }
}
...
push_back(U &&u)
{
  if (prio_q_likely(end_ & block_mask))
  {
    A::construct(*this, ptr_ + end_, std::forward<U>(u));
    return end_++;

  }
  if (prio_q_unlikely(end_ == storage_size_))
  {
...
{
  assert(end_);
  A::destroy(*this, ptr_ + --end_);
  end_ -= (end_ & block_mask) == 1;
}
...
grow(U &&u)
{
  auto desired_size = storage_size_ ? storage_size_ * 2 : block_size * 16;
  auto ptr          = A::allocate(*this, desired_size, ptr_);
  auto idx          = 0;
  try
  {
    A::construct(*this, ptr + end_ + 1, std::forward<U>(u));
    idx = end_ + 1;
    if (storage_size_)
    {
      move_to(ptr_, end_, ptr);
      A::deallocate(*this, ptr_, storage_size_);
    }
    ptr_          = ptr;
    storage_size_ = desired_size;
    end_          = idx + 1;
    return idx;
  }
  catch (...)
  {
    ...
    A::deallocate(*this, ptr, storage_size_);
    throw;
  }
}
...
{
  for (std::size_t i = 1; prio_q_likely(i < s); ++i)
  {
    if (prio_q_likely(i & block_mask))
    {
...
{
public:
  payload(Allocator const &alloc = Allocator{ }) : storage_(alloc) { }
  template <typename U>
  void push_back(U &&u) { storage_.push_back(std::forward<U>(u)); }
  void pop_back() { storage_.pop_back(); }
  V &top() { return storage_[1]; }
  V &back() { return storage_.back(); }
  void store(std::size_t idx, V &&v) { storage_[idx] = std::move(v); }
  void move(std::size_t from, std::size_t to)
  {
    storage_[to] = std::move(storage_[from]);
  }
private:
  skip_vector<V, block_size, Allocator> storage_;
};
...
private:
  ...
  prio_q_internal::skip_vector<T, block_size, Allocator> storage_;
...
push_key(U &&key)
{
  auto hole_idx = storage_.push_back(std::forward<U>(key));
  auto tmp      = std::move(storage_.back());
  auto val      = std::move(P::back());

  while (prio_q_likely(hole_idx != 1U))
  {
    auto parent = address::parent_of(hole_idx);
    auto &p     = storage_[parent];
    if (prio_q_likely(!sorts_before(tmp, p))) break;
    storage_[hole_idx] = std::move(p);
    P::move(parent, hole_idx);
    hole_idx = parent;
  }
  storage_[hole_idx] = std::move(tmp);
...
noexcept(std::is_nothrow_destructible<T>::value)
{
  assert(!empty());
  std::size_t idx      = 1;
  auto const  last_idx = storage_.size() - 1;
  for (; ;)
  {
    auto lc = address::child_of(idx);
    if (prio_q_unlikely(lc > last_idx)) break;
    auto const sibling_offset = prio_q_unlikely(address::is_block_leaf(idx))
                                ? address::block_size : 1;
    auto       rc             = lc + sibling_offset;
    auto       i              =
                   rc < last_idx && !sorts_before(storage_[lc], storage_[rc]);
    auto       next           = i ? rc : lc;
    storage_[idx] = std::move(storage_[next]);
    P::move(next, idx);
    idx = next;
  }
  if (prio_q_likely(idx != last_idx))
  {
    auto last     = std::move(storage_.back());
    auto last_val = std::move(P::back());
    while (prio_q_likely(idx != 1))
    {
      auto parent = address::parent_of(idx);
      if (prio_q_likely(!sorts_before(last, storage_[parent]))) break;
      storage_[idx] = std::move(storage_[parent]);
      P::move(parent, idx);
      idx = parent;
    }
    storage_[idx] = std::move(last);
...
noexcept(noexcept(std::declval<T&>() = std::declval<T&&>()))
{
  ...
  for (;;)
  {
    auto lc = address::child_of(idx);
    if (prio_q_unlikely(lc > last_idx)) break;
    auto const sibling_offset = prio_q_unlikely(address::is_block_leaf(idx)) ? address::block_size : 1;
    auto rc = lc + sibling_offset;
    auto i = rc <= last_idx && !sorts_before(storage_[lc], storage_[rc]);
    auto next = i ? rc : lc;
    if (sorts_before(t, storage_[next])) break;
    storage_[idx] = std::move(storage_[next]);
    P::move(next, idx);
    idx = next;
  }
  storage_[idx] = std::move(t);
  return idx;
}
...
noexcept
{
  return storage_.size()
      - (storage_.size() + address::block_size - 1) / address::block_size;
}
...
noexcept
{
  if (prio_q_likely(!is_block_leaf(node_no)))
  {
...
noexcept
{
  ...
  if (prio_q_likely(!is_block_root(node_no)))
  {
...
#undef prio_q_likely
#undef prio_q_unlikely

#endif //EFFICIENT_BHEAP_PRIO_QUEUE_HPP
